 ...
 */
const ERROR_CATEGORIES: Record<number, string> = {
 ...
 */
function decodeAbortCode(code: string | number): {
  category: string;
  reason: number;
  hex: string;
} | undefined {
...
function parseVmStatus(vmStatus?: string): {
  ...
  // "ABORTED with code <code> and message <msg> in <location>"
  // "ABORTED with code <code> in <location>"
  const keptAbortMatch = vmStatus.match(
    /^status\s+ABORTED\s+with\s+code\s+(\d+)(?:\s+and\s+message\s+(.+?))?\s+in\s+(.+)$/i,
  );
  if (keptAbortMatch) {
  ...
  const keptExecMatch = vmStatus.match(
    /EXECUTION_FAILURE\s+at\s+bytecode\s+offset\s+(\d+)\s+in\s+function\s+index\s+(\d+)\s+in\s+(.+?)(?:\s+with\s+error\s+message\s+(.+))?$/i,
  );
  if (keptExecMatch) {
    return {
  ...
  // "status ABORTED of type Execution with sub status <code>"
  // "status ABORTED of type Execution with sub status <code> with message <msg>"
  const sessionAbortMatch = vmStatus.match(
    /status\s+ABORTED\s+of\s+type\s+\w+\s+with\s+sub\s+status\s+(\d+)(?:\s+with\s+message\s+(.+))?$/i,
  );
  ...
  const sessionErrorMatch = vmStatus.match(
    /status\s+(\w+)\s+of\s+type\s+\w+(?:\s+with\s+message\s+(.+))?$/i,
  );
